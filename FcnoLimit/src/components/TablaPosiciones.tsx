...
import { arrowForward, trophyOutline } from 'ionicons/icons';
import '../pages/home/campeonato/CampeonatoPage.css';
...
const TablaPosiciones = ({ showHeader = true, showTabs = true, showSelectors = true }) => {
  ...
  const [selectedDivisionEquipo, setSelectedDivisionEquipo] = useState(1);
  ...
  const colorForma = (f: string) => {
    if (f === 'G') return { background: '#4caf50', color: '#fff' };
    if (f === 'E') return { background: '#ffc107', color: '#fff' };
    if (f === 'P') return { background: '#f44336', color: '#fff' };
    return {};
  };

  const calcularForma = (partidos: any[], equipoId: number) => {
    return partidos.map((p) => {
      if (p.goles_local == null || p.goles_visitante == null) return 'E';
    ...
    <div className="content-section standings-section animate-on-scroll">
      {showHeader && (
        <div className="section-header">
          ...
          </h2>          <a href="/campeonatos" className="view-all-link">
            Ver completa <IonIcon icon={arrowForward} />
          </a>
        </div>
      )}      {showTabs && showSelectors && (
