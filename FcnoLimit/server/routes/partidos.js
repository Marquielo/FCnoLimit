...
module.exports = (pool) => {
  ...
  // Obtener los últimos 5 partidos jugados de un equipo en una división (público)
  router.get('/historial/ultimos5/:equipo_id/:division_id', async (req, res) => {
    const equipoId = parseInt(req.params.equipo_id, 10);
    const divisionId = parseInt(req.params.division_id, 10);
    if (isNaN(equipoId) || isNaN(divisionId)) {
      return res.status(400).json({ error: 'ID de equipo o división inválido' });
    }
    try {
      const result = await pool.query(
        `SELECT
